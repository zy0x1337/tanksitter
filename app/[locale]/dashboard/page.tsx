...
import { Plus, Settings, QrCode, UserCog, Waves, ArrowRight } from 'lucide-react'
      ...
      <div className="max-w-6xl mx-auto space-y-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-4 border-b border-border/40">
          ...
          <div className="flex items-center gap-3">
             ...
             <Link href={`/${locale}/dashboard/settings`}>
                <Button variant="outline" size="icon" className="rounded-xl border-border/60 hover:bg-secondary hover:border-border transition-all">
        ...
        {tanks && tanks.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {tanks.map((tank) => (
              <div key={tank.id} className="group relative bg-card/60 dark:bg-card/40 backdrop-blur-md rounded-[2rem] border border-border/50 hover:border-blue-500/30 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/10 overflow-hidden">
                ...
                <div className="p-8 relative z-10 flex flex-col h-full">
                  <div className="flex justify-between items-start mb-6">
                      <div className="w-14 h-14 bg-gradient-to-br from-white to-blue-50 dark:from-slate-800 dark:to-slate-900 rounded-2xl flex items-center justify-center shadow-inner border border-white/20 dark:border-white/5">
                      ...
                      <ShareDialog 
                        ...
                        triggerButton={
                           <Button variant="ghost" size="icon" className="rounded-xl text-muted-foreground hover:text-foreground hover:bg-white/50 dark:hover:bg-white/10">
          ...
          // Empty State
          <div className="flex flex-col items-center justify-center py-32 px-4 text-center border-2 border-dashed border-border rounded-[3rem] bg-secondary/5">
            <div className="w-24 h-24 bg-gradient-to-tr from-blue-100 to-cyan-100 dark:from-blue-900/30 dark:to-cyan-900/30 rounded-3xl flex items-center justify-center mb-8 shadow-inner animate-float">
              <span className="text-5xl opacity-80 drop-shadow-sm">🐠</span>
            </div>
            <h3 className="text-2xl font-bold text-foreground mb-3">{t('no_tanks')}</h3>
            <p className="text-muted-foreground mb-8 max-w-sm text-lg leading-relaxed">
              {t('no_tanks_desc')}
            </p>
            <Link href={`/${locale}/dashboard/new`}>
              <Button size="lg" className="rounded-2xl h-14 px-8 text-lg bg-blue-600 hover:bg-blue-500 text-white shadow-xl shadow-blue-500/20 hover:-translate-y-1 transition-all">
                <Plus className="w-5 h-5 mr-2" />
                {t('new_tank_button')}
              </Button>
            </Link>
